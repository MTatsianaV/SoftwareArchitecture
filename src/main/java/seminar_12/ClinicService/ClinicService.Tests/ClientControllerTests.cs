...
namespace ClinicService.Tests
{
    public class ClientControllerTests
    {
        ...
        public void UpdateClientTest()
        {
                    ...
                    _mockClientRepository
                        .Setup(repository => repository.Update(clientId, It.IsNotNull<Client>()))
                        .Returns(true);

                    // [2] Исполнение тестируемого метода
                    var operationResult = _clientController.Update(clientId, updatedClient);

                    // [3] Подготовка эталонного результата и проверка результата
                    Assert.IsType<OkResult>(operationResult);
                    _mockClientRepository.Verify(repository =>
                        repository.Update(clientId, It.IsNotNull<Client>()), Times.AtLeastOnce);
        }
         ...
         public void DeleteClientTest()
         {
                    ...
                    _mockClientRepository
                        .Setup(repository => repository.Delete(clientId))
                        .Returns(true);
